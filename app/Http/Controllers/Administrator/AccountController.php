...
class AccountController extends Controller
{
	...
	public function profileUpdate(request $request){
		...
		$validator = Validator::make($request->all(), [
			'name' => 'required|min:3',
			'email' => 'required|email|unique:phat_administrator,email,'.$id,
	...
	public function add(request $request){
		...
		$validator = Validator::make($request->all(), [
			'name' => 'required|min:3',
			'email' => 'required|email|unique:phat_administrator,email',
		], $message);
		...
		$newpass = Hash::make('atiga@'.str_random(5));
		...
		Mail::send('mail.new-user', ['data' => $data], function($message) use ($data) {
